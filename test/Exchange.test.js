...
contract('Exchange', ([deployer, feeAccount, user1, user2]) => {
    ...
    describe('withdrawing tokens', async () => {
        ...
        describe('failure', async () => {
            ...
            // Attempt to withdraw tokens without depoiting any first
        ...
        describe('cancelling orders', async () => {
            ...
            describe('success', async () => {
                ...
                it('emits a "Cancel" event', async () => {
                    ...
                    event.tokenGive.should.equal(ETHER_ADDRESS, 'tokenGive is correct')
                    event.amountGive.toString().should.equal(ether(1).toString(), 'amountGive is correct')
                    event.timestamp.toString().length.should.be.at.least(1, 'timestampis present')
                })
            })
            describe('failure', async () => {
                it('rejects invalid order ids', async () => {
    ...
    describe('fillOrder()', () => {
        describe('Check balances after filling user1 buy Tokens order', () => {
            beforeEach(async () => {
                ...
                await exchange.depositEther({ from: user1, value: ether(1)})
                // user 1 create order to buy 10 tokens for 1 ETHER
